...
impl ColumnarList {
    pub fn new(columns: Vec<ListColumn>, row_height: f32, row_gap: f32) -> Self {
        Self {
            base: Widget::new(),
    ...
    pub fn get_column_bounds(&self, list_w: f32) -> Vec<(f32, f32)> {
        let mut bounds = vec![(0.0, 0.0); self.columns.len()];
            ...
            match col.width {
                ...
                ColumnWidth::RightOffset(offset) => {
                    ...
                    for j in (i + 1)..self.columns.len() {
                        if let ColumnWidth::RightOffset(o) = self.columns[j].width {
                            next_x = list_w - o;
                            break;
                        }
                    }
        ...
        for (_, col) in self.columns.iter().enumerate() {
    ...
    pub fn update_bounds(&mut self) {
        let item_height_full = self.row_height + self.row_gap;
...
impl Element for ColumnarList {
    ...
    fn on_cursor_moved(&mut self, px: f32, py: f32, ctx: &mut UiContext) -> bool {
        ...
        for idx in 0..self.rows.len() {
            ...
            if let Some(draw_y) = self.scroll_box.get_item_draw_y(virtual_y, self.row_height) {
                if px >= x + 2.0 && px <= x + w - 2.0 && py >= draw_y && py <= draw_y + self.row_height {
    ...
    fn extra_quads(&self) -> Vec<(f32, f32, f32, f32, [f32; 4])> {
        ...
        for idx in 0..self.rows.len() {
            ...
            if let Some(draw_y) = self.scroll_box.get_item_draw_y(virtual_y, self.row_height) {
                ...
                if bg_color[3] > 0.001 {
                    quads.push((x + 2.0, draw_y, w - 4.0, self.row_height, bg_color));
                }
            }
        }
        quads
    ...
    fn text_labels_with_font_and_bounds(&self, _ctx: &UiContext) -> Vec<(TextLabel, Option<String>, Option<[f32; 4]>)> {
        ...
        for idx in 0..self.rows.len() {
            ...
            if let Some(draw_y) = self.scroll_box.get_item_draw_y(virtual_y, self.row_height) {
                ...
                let y_text_12 = crate::layout::center_text_y(draw_y, self.row_height, 12.0);
                let y_text_11 = crate::layout::center_text_y(draw_y, self.row_height, 11.0);
    ...
    fn keyboard_input(&mut self, event: &KeyEvent, ctx: &mut UiContext) -> bool {
        ...
        if event.state == ElementState::Pressed {
            ...
            if let Some(next) = next_selected {
                ...
                } else if item_y + self.row_height > self.scroll_box.scroll_y + viewport_h {
                    self.scroll_box.scroll_y = item_y + self.row_height - viewport_h;
                }
...
unsafe impl Send for ColumnarList {}
unsafe impl Sync for ColumnarList {}
